...
function addlog($operation_id='')
{
    ...
    if($web_config['is_log'] == 1) {
        ...
        $datas=\think\Db::name('admin_menu')->where($url)->where('parameter',$string)->value('id');
        if (!$datas) {
            $data['admin_menu_id']=\think\Db::name('admin_menu')->where($url)->value('id');
        }else{
            $data['admin_menu_id']=\think\Db::name('admin_menu')->where($url)->where('parameter',$string)->value('id');
        }
